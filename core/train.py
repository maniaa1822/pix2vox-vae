...
def train_net(cfg):
    ...
    for epoch_idx in range(init_epoch, cfg.TRAIN.NUM_EPOCHES):
        ...
        for batch_idx, (taxonomy_names, sample_names, rendering_images,
            ...
            raw_features, generated_volumes = decoder(z)
            ...
            train_writer.add_scalar('EncoderDecoder/KLDivergence', kl_divergence.item(), n_itr)
            ...
            batch_end_time = time()
            print(
                '[INFO] %s [Epoch %d/%d][Batch %d/%d] BatchTime = %.3f (s) DataTime = %.3f (s) EDLoss = %.4f KLDiv = %.4f RLoss = %.4f'
                % (dt.now(), epoch_idx + 1, cfg.TRAIN.NUM_EPOCHES, batch_idx + 1, n_batches, batch_time.val,
                   data_time.val, encoder_loss.item(), kl_divergence.item(), refiner_loss.item()))

        # Append epoch loss to TensorBoard
        train_writer.add_scalar('EncoderDecoder/EpochLoss', encoder_losses.avg, epoch_idx + 1)
        train_writer.add_scalar('EncoderDecoder/EpochKLDivergence', kl_divergences.avg, epoch_idx + 1)
        ...
        # Tick / tock
        epoch_end_time = time()
        print('[INFO] %s Epoch [%d/%d] EpochTime = %.3f (s) EDLoss = %.4f KLDiv = %.4f RLoss = %.4f' %
              (dt.now(), epoch_idx + 1, cfg.TRAIN.NUM_EPOCHES, epoch_end_time - epoch_start_time, encoder_losses.avg,
               kl_divergences.avg, refiner_losses.avg))
